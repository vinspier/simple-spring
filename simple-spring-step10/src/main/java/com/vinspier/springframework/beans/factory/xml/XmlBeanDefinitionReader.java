...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
    ...
    protected void doParseElementAndRegistry(Element element) throws ClassNotFoundException {
        String id = element.getAttribute(XmlTagEnum.ID.tag());
        String name = element.getAttribute(XmlTagEnum.NAME.tag());
        String className = element.getAttribute(XmlTagEnum.CLASS_TAG.tag());
        String initMethodName = element.getAttribute(XmlTagEnum.INIT_METHOD.tag());
        String destroyMethodName = element.getAttribute(XmlTagEnum.DESTROY_METHOD.tag());
        String scope = element.getAttribute(XmlTagEnum.SCOPE.tag());
        // 类对象
        ...
        for (int i = 0; i < propertyNodes.getLength(); i++) {
            ...
            if (! (node instanceof Element) || !XmlTagEnum.PROPERTY.tag().equals(node.getNodeName())) {
                continue;
            }
            Element propertyEle = (Element) node;
            String attrName = propertyEle.getAttribute(XmlTagEnum.NAME.tag());
            String attrValue = propertyEle.getAttribute(XmlTagEnum.VALUE.tag());
            // 对象引用
            String ref = propertyEle.getAttribute(XmlTagEnum.REF.tag());
